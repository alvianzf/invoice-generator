...
export const generateInvoicePDF = (invoiceData: InvoiceData) => {
  ...
  const total = invoiceData.items.reduce((sum, item) => {
    const amount = Number(
      item.amount.replace(/\./g, "").replace(/[^0-9-]/g, "")
    );
  ...
  const paymentY = (doc as any).lastAutoTable.finalY + 10; // Space after total
  ...
  const contactY = yPos + 10; // Space after payment details

  doc.setFontSize(10);
  doc.setFont("helvetica", "italic");
  doc.setTextColor(80, 80, 80);
  doc.text(
    `* For any inquiries or payment-related issues, contact ${
      invoiceData.contactEmail || "your-email@example.com"
    } or ${invoiceData.contactPhone || "your-phone-number"}.`,
    14,
    contactY
  );
