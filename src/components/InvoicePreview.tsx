    ...
    <div className="bg-white p-8 shadow-lg rounded-lg max-w-4xl mx-auto border border-gray-200">
      ...
      <div className="bg-gray-50 -m-8 p-8 mb-8 border-b border-gray-200">
        <div className="flex justify-between">
          <div>
            <p className="text-gray-700 font-medium">NO: {invoiceData.invoiceNumber}</p>
      ...
      <div className="flex justify-between mb-12">
        <div className="w-1/2 pr-4">
          <h2 className="text-lg font-semibold mb-3 text-blue-800">Billed to:</h2>
          <p className="font-medium">{invoiceData.billedToCompanyName}</p>
          <p className="text-gray-600">{invoiceData.billedToAddress}</p>
          <p className="text-gray-600">Company ID: {invoiceData.billedToCompanyId}</p>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-blue-800 text-white">
              <th className="border border-blue-700 px-4 py-3 text-left rounded-tl-md">Item</th>
              <th className="border border-blue-700 px-4 py-3 text-right">Quantity</th>
              <th className="border border-blue-700 px-4 py-3 text-right">Price</th>
              <th className="border border-blue-700 px-4 py-3 text-right rounded-tr-md">Amount</th>
            </tr>
          </thead>
          <tbody>
            {invoiceData.items.map((item, index) => (
              <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                ...
                <td className="border border-gray-200 px-4 py-3 text-right">{item.price}</td>
                <td className="border border-gray-200 px-4 py-3 text-right">{item.amount}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr>
              ...
              <td className="border border-gray-200 px-4 py-3 text-right font-bold bg-gray-100">Total</td>
      ...
      <div className="mb-8 bg-gray-50 p-5 rounded-md">
        <h2 className="text-lg font-semibold mb-3 text-blue-800 border-b border-blue-200 pb-1">Payment Details</h2>
